...
async def on_message_text(message: AbstractIncomingMessage) -> None:
    async with message.process(requeue=False):
        message_json = json.loads(message.body.decode())
        print(f" [x] Received message {message_json}")
        parser = PlaintextParser.from_string(
            message_json["text"], Tokenizer(message_json["language"])
        )
...
async def main() -> None:
    ...
    async with connection:
        ...
        # Declaring queue
        queue_text = await channel.declare_queue(
            "task_queue_text",
            durable=True,
        )

        # Start listening the queue with name 'task_queue'
        await queue_text.consume(on_message_text)
